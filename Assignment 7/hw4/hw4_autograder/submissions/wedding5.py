...
def swap_without_tail(guests_lst, guests_perms, len_of_original):
    ...
    for i in range(len_of_original - 1):
        swapListTemp = []
        swapListTemp = swapListTemp + guests_lst
        tempGuesList = [] + swapListTemp

        swapped_val = swapGuests(swapListTemp, i)
    ...
    for i in range(a, b):
        ...
        for j in range(len_of_original):
            if(tempGuesList[j] != swapped_val_lst[j]):
                val = j

        for z in range(val+1, len_of_original - 1):
            svl = [] + swapped_val_lst
            swapped_val = swapGuests(svl, z)
            guests_perms.append(swapped_val)
